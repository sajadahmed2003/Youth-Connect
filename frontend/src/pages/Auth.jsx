...
const Auth = ({ onLogin }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`http://localhost:5003${endpoint}`, {
        method: 'POST',
    ...
    <div className="auth-portal-cyber" style={{
      ...
      <div className="cyber-card" style={{ width: '100%', maxWidth: '450px', padding: '50px', border: '1px solid rgba(12, 166, 166, 0.2)', boxShadow: '0 0 50px rgba(0,0,0,0.8)' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '40px' }}>
          ...
          <h1 style={{ fontSize: '2rem', fontWeight: '900', letterSpacing: '2px', color: 'white', margin: 0 }}>CAMPAIGN CONNECT</h1>
          <p style={{ color: '#4ade80', fontSize: '0.8rem', fontWeight: 'bold', letterSpacing: '4px', textTransform: 'uppercase', marginTop: '10px' }}>ACCESS PORTAL</p>
        </div>
        ...
        {isLogin && (
          <button
            ...
            style={{
              ...
              background: 'rgba(12, 166, 166, 0.1)', border: '1px solid #0ca6a6',
              borderRadius: '10px', color: '#4ade80', fontWeight: '900',
              fontSize: '0.75rem', cursor: 'pointer', display: 'flex',
              alignItems: 'center', justifyContent: 'center', gap: '10px'
            }}
          >
            <ShieldAlert size={18} /> DEV ONLY: LOGIN AS SUPER ADMIN
          </button>
        )}
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginBottom: '20px' }}>
            <div style={{ color: '#64748b', fontSize: '0.7rem', fontWeight: 'bold', marginBottom: '8px', letterSpacing: '1px' }}>GLOBAL MAIL ROUTE</div>
            <div style={{ position: 'relative' }}>
          ...
          <div style={{ marginBottom: '30px' }}>
            <div style={{ color: '#64748b', fontSize: '0.7rem', fontWeight: 'bold', marginBottom: '8px', letterSpacing: '1px' }}>ENCRYPTION KEY</div>
            <div style={{ position: 'relative' }}>
          ...
          <button id="main-submit-btn" type="submit" className="cyber-card" style={{ width: '100%', padding: '18px', background: 'linear-gradient(90deg, #0ca6a6 0%, #115e5e 100%)', border: 'none', color: 'white', fontWeight: '900', letterSpacing: '2px', cursor: 'pointer', boxShadow: '0 0 20px rgba(12, 166, 166, 0.3)', marginBottom: '25px', opacity: loading ? 0.6 : 1 }} disabled={loading}>
            {loading ? 'SYNCHRONIZING...' : isLogin ? 'BYPASS SECURITY' : 'AUTHORIZE ACCOUNT'}
          </button>

          <div style={{ textAlign: 'center', fontSize: '0.85rem' }}>
            <span style={{ color: '#64748b' }}>{isLogin ? "New user in the ecosystem?" : "Already possess clearance?"}</span>
            <span onClick={() => setIsLogin(!isLogin)} style={{ color: '#0ca6a6', cursor: 'pointer', fontWeight: 'bold', marginLeft: '10px', textDecoration: 'underline' }}>
              {isLogin ? 'Initialize- SIGN UP' : 'Return to Login'}
            </span>
          </div>
        </form>
      </div>
